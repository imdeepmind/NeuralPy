---
title: "BatchNorm3D Layer"
metaTitle: "BatchNorm3D Layer | Normalization Layers | NeuralPy Deep Learning Library"
metaDescription: "Applies Batch Normalization over a 5D input."
---

# BatchNorm3D

```python
neuralpy.layers.BatchNorm3D(num_features, eps=1e-05, momentum=0.1, affine=True, track_running_status=True, name=None)
```

Applies Batch Normalization over a 5D input.

To learn more about BatchNorm3D layers, please check [pytorch documentation](https://pytorch.org/docs/stable/nn.html#batchnorm3d).
...
from neuralpy.layers import Conv3D, BatchNorm3D

# Making the model
model = Sequential()
model.add(Conv3D(filters=8, kernel_size=3, input_shape=(1, 28, 28, 28), stride=1, name="first cnn"))
model.add(BatchNorm3D(eps=1e-05, momentum=0.1, affine=True, track_running_status=True, name="batch norm"))
```
