...
metaTitle: "BatchNorm1D Layer | Normalization Layers | NeuralPy Deep Learning Library"
metaDescription: "Applies Batch Normalization over a 2D or 3D input."
---

# BatchNorm1D

```python
neuralpy.layers.BatchNorm1D(num_features, eps=1e-05, momentum=0.1, affine=True, track_running_status=True, name=None)
```

Applies Batch Normalization over a 2D or 3D input

To learn more about BatchNorm1D layers, please check [pytorch documentation](https://pytorch.org/docs/stable/nn.html#batchnorm1d).
...
from neuralpy.layers import Conv1D, BatchNorm1D

# Making the model
model = Sequential()
model.add(Conv1D(filters=8, kernel_size=3, input_shape=(1, 28), stride=1, name="first cnn"))
model.add(BatchNorm1D(eps=1e-05, momentum=0.1, affine=True, track_running_status=True, name="batch norm"))
```
